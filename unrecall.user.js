...
(function () {
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // PART 1 – Page-world injection
  //
  // Chrome MV3 isolates Tampermonkey's JS context from the page's real JS
  // environment, so overriding window.fetch directly from a userscript never
  // intercepts what the page actually calls.
  //
  // Fix: stringify this function and inject it as a <script> tag.  A <script>
  // element always runs in the page's native JS context — no isolation, no
  // sandboxing.  Captured data is sent back to Part 2 via CustomEvent.
  // ══════════════════════════════════════════════════════════════════════════

  function pageWorld() {
    const ENDPOINT_RE = /\/chat\/completions?\b/i;
    const CAPTURE_EVENT = 'unrecall:session';
    ...
    function createProcessor() {
      ...
      function handlePacket(pkt) {
        // Continuation token or init packet (no p, no o)
      ...
      function finish() {
        ...
        if (!r) return;
        // Only keep THINK and RESPONSE fragments (skip TOOL_SEARCH, TOOL_OPEN, etc.)
    ...
    // ── fetch intercept (runs in the page's real JS context) ─────────────────
  ...
  // Inject pageWorld() into the page's native JS context via a <script> tag.
  // This is the only approach that reliably works across Chrome MV2/MV3.
  const injected = document.createElement('script');
  injected.textContent = `(${pageWorld.toString()})()`;
  document.documentElement.prepend(injected);
  injected.remove();

  // ══════════════════════════════════════════════════════════════════════════
  // PART 2 – Tampermonkey-world UI
  //
  // Receives captured sessions via CustomEvent from Part 1.
  // Handles all DOM/CSS work (the TM sandbox has full DOM access).
  // ══════════════════════════════════════════════════════════════════════════

  const sessions = [];
  let ui = null;

  window.addEventListener('unrecall:session', (e) => {
    try {
      sessions.push(JSON.parse(e.detail));
      renderPanel();
    } catch { /* skip */ }
  });
  ...
  // Initialize UI immediately so the tab is visible as a health indicator
  if (document.body) {
    ensureUI();
  } else {
